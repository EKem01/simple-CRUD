...
if (isset($_POST['submit'])) {
    ...
    $check_mobile = $conn->prepare("select * from people where mobile = '" . $mobile . "'"); // to check duplicate
    $check_mobile->execute();
    if ($check_mobile->rowCount() > 0) {
        header("Location: index.php?message=Duplicate entry");
    } else {
        $insert_query = $conn->prepare("insert into people (name, email, mobile) values (:name,:email,:mobile)"); //to insert data
        try {
