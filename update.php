...
if (isset($_POST['submit'])) {
    ...
    $check_mobile = $conn->prepare("select * from people where mobile = '" . $mobile . "' and id not in ('".$id."')"); // to check duplicate
    $check_mobile->execute();
    if ($check_mobile->rowCount() > 0) {
        header("Location: index.php?message=Duplicate entry");
    } else {
        $insert_query = $conn->prepare("update people set name = :name, email=:email, mobile=:mobile where id = :id"); //to insert data
        try {
